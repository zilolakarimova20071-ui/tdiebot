...
async def sync_groups_now(user: str = Depends(check_auth)):
    from bot import group_url_scraper
...
async def toggle_scanning(user: str = Depends(check_auth)):
...
async def scan_teachers_now(user: str = Depends(check_auth)):
...
async def new_items_page(request: Request, user: str = Depends(check_auth)):
...
async def dismiss_new_item(item_id: int, user: str = Depends(check_auth)):
...
async def clear_new_items(item_type: str, user: str = Depends(check_auth)):
