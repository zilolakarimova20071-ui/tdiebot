...
Bunga ikki qoida bilan erishiladi:
  ...
  2) Aks holda, so'zlar prefiks sifatida yoki difflib orqali "yetarlicha
     o'xshash" (ratio >= 0.75) bo'lsa mos deb hisoblanadi.
"""
...
def _word_matches(query_word: str, name_word: str) -> bool:
    ...
    name_word = name_word.lower().rstrip(".")
    if not query_word or not name_word:
        return False

    # Bittasi bosh harf (initsial) bo'lsa, faqat birinchi harf mos
    # kelishi kifoya (masalan qidiruv "dilbar", saqlangan "d").
    if len(name_word) == 1 or len(query_word) == 1:
        return name_word[0] == query_word[0]

    if name_word.startswith(query_word) or query_word.startswith(name_word):
        return True

    # Yengil imlo farqiga chidamli (masalan "dilbar" vs "dilber").
