...
async def log_chat_message(user_id: int, direction: str, text: str):
    now = _now()
    if USE_POSTGRES:
        async with _pg_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO chat_messages (user_id, direction, text, created_at) VALUES ($1, $2, $3, $4)",
        ...
        with _sqlite_lock:
            conn.execute(
                "INSERT INTO chat_messages (user_id, direction, text, created_at) VALUES (?, ?, ?, ?)",
                (user_id, direction, text, now.isoformat(timespec="seconds")),
            )
            conn.commit()
        ...
        with _sqlite_lock:
            conn.execute(
                """INSERT INTO admins (username, password_hash, created_at) VALUES (?, ?, ?)
                   ON CONFLICT(username) DO UPDATE SET password_hash=excluded.password_hash""",
                (username, pw_hash, now.isoformat(timespec="seconds")),
            )
            conn.commit()
...
async def get_all_admins() -> list:
    if USE_POSTGRES:
        async with _pg_pool.acquire() as conn:
            rows = await conn.fetch("SELECT username, created_at FROM admins ORDER BY created_at")
            return [dict(r) for r in rows]
    else:
        conn = _get_sqlite_conn()
        cur = conn.execute("SELECT username, created_at FROM admins ORDER BY created_at")
        return [dict(r) for r in cur.fetchall()]
